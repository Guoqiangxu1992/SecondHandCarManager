...
function search(){
	...
	var aj = $.ajax( {    
	    url:'${xgq}/quartz/getQuartzList.do',// 跳转到 action    
	    type:'POST',    
	    dataType:'json',  
	    data:param,  
	    success:function(data) {
	    	var str = eval(data);
	    	 if(str!=''){
        		 ...
        		 $.each(str.rows,function(n,value){
        			    html = "";
	     	            html += '<tr>';
	     	            html += '<td><p class="long_txt">'+value.jobId+'</p></td>';
	     	            html += '<td> <p class="long_txt">'+value.jobName+'</p></td>';
	     				html += '<td><p class="long_txt">'+value.jobGroup+'</p></td>';
	     				...
	     				html += '<td><p class="long_txt">'+value.cronExpression+'</p></td>';
	     				html += '<td><p class="long_txt">'+formartDate("yyyy-MM-dd HH:mm:ss",
								value.createTime.time) +'</p></td>';
	     				html += '<td><p class="long_txt">'+formartDate("yyyy-MM-dd HH:mm:ss",
								value.updateTime.time) +'</p></td>';
	     				html += '<td><p class="long_txt">'+value.creator+'</p></td>';
		...
		<div class="col-lg-12">
			<section class="panel">
				...
				<table class="table table-striped table-advance table-hover">
					<thead>
						<tr>
							<th><i class="icon-bookmark"></i>任务编号</th>
							<th><i class="icon-bookmark"></i>任务名称</th>
							<th><i class="icon-bookmark"></i>任务组</th>
							<th><i class="icon-bookmark"></i>任务状态</th>
							<th><i class="icon-bookmark"></i>Con表达式</th>
