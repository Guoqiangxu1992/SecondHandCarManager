...
//					edit             : true,  						      // 是否可以编辑文件（裁剪）
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
		            ...
		            html += '					<div class="close_btn">关闭</div>';
		            html += '				</div>';
		            html += '			</div>';
					...
					if(newStr.split(".")[0] != null){
						...
						if(jQuery.inArray(type, para.fileType) > -1){
							...
							if (file.size >= para.fileSize) {
								alert(para.fileSize+'您这个"'+ file.name +'"文件大小过大'+file.size);	
							} else {
								// 在这里需要判断当前所有文件中
								arrFiles.push(file);	
							}
...
//								}
			...
			this.addEvent = function(){
				...
				$(".close_btn").bind("click", function(e){
					// 判断当前是否有文件需要上传
					var url = "/SecondHandCarManager/car/EditCarManager.do";
					submitform(url);
				});
